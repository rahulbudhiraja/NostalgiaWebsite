#!/bin/bash
#
# Developed by Fred Weinhaus 9/5/2011 .......... 8/22/2012
#
...
inner=30			# inner radius of vignette where not darkened as percent of image dimension
outer=150		# outer radius of vignette where darkest
dark=30			# graylevel of darkest part of vignette in range 0 to 100
...
# get im_version
im_version=`/usr/local/bin/convert -list configure | \
...
# test input image
/usr/local/bin/convert -quiet -regard-warnings "$infile" $grayish +repage "$tmpA1" ||
	errMsg "--- FILE $infile DOES NOT EXIST OR IS NOT AN ORDINARY FILE, NOT READABLE OR HAS ZERO SIZE ---"


# setup vignette
ww=`/usr/local/bin/convert $tmpA1 -format "%w" info:`
hh=`/usr/local/bin/convert $tmpA1 -format "%h" info:`
wwo=`/usr/local/bin/convert xc: -format "%[fx:$outer*$ww/100]" info:`
hho=`/usr/local/bin/convert xc: -format "%[fx:$outer*$hh/100]" info:`
mwh=`/usr/local/bin/convert xc: -format "%[fx:$outer*min($ww,$hh)/100]" info:`
if [ "$inner" = "0" ]; then
	mlevel=""
else
	inner=$((100-inner))
...
if [ "$barrel" = "0" ]; then 
	...
	distort=`/usr/local/bin/convert xc: -format "%[fx:$barrel/100]" info:`
	test=`/usr/local/bin/convert xc: -format "%[fx:sign($distort)<0?1:0]" info:`
	if [ $test -eq 1 ]; then
		distort1=`/usr/local/bin/convert xc: -format "%[fx:1+2*$distort]" info:`
	else
		distort1=`/usr/local/bin/convert xc: -format "%[fx:1-2*$distort]" info:`
	fi
...
if [ "$double" = "+0,+0" -o "$double" = "-0,-0"  ]; then
	...
	axroll=`/usr/local/bin/convert xc: -format "%[fx:abs($xroll)]" info:`
	ayroll=`/usr/local/bin/convert xc: -format "%[fx:abs($yroll)]" info:`
	rolling3="-shave ${axroll}x${ayroll}"
fi
#echo "rolling1=$rolling1; rolling2=$rolling2; rolling3=$rolling3"


/usr/local/bin/convert \( $tmpA1 \( $rolling1 \) $rolling2 $contrasting $iblurring $sharpening $distorting \) \
	\( +clone $oblurring \) \
	...
	-delete 0-2 +swap -compose multiply -composite -limit thread 4 $rolling3 $outfile
